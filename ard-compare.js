...
if (!networkResultsPath || !ardPath) {
  ...
  console.log('📋 ARD Comparison Tool - Compare test results against ARD requirements\n');
  ...
  console.log('  Enable/disable report types in config/main.js → ARD_REPORT_GENERATION');
  process.exit(1);
}

// Check if files exist
if (!fs.existsSync(networkResultsPath)) {
  console.error(`❌ Network results file not found: ${networkResultsPath}`);
  process.exit(1);
}

if (!fs.existsSync(ardPath)) {
  console.error(`❌ ARD file not found: ${ardPath}`);
  process.exit(1);
}
...
console.log('🔍 Starting ARD Comparison...\n');
console.log(`📊 Test Results: ${networkResultsPath}`);
...
try {
  ...
  console.log('═══════════════════════════════════════════════════════');
  console.log(`   📈 Coverage:         ${comparisonResults.summary.coverage}%`);
  console.log(`   ✅ Matching:         ${comparisonResults.summary.matchingCount} events`);
  console.log(`   ❌ Missing:          ${comparisonResults.summary.missingCount} events`);
  console.log(`   ➕ Extra:            ${comparisonResults.summary.extraCount} events`);
  console.log(`   ⚠️  Mismatches:      ${comparisonResults.summary.parameterMismatchCount} events`);
  console.log(`   📋 Total ARD:        ${comparisonResults.summary.totalARDEvents} events`);
  console.log(`   🧪 Total Test:       ${comparisonResults.summary.totalTestEvents} events`);
  console.log('═══════════════════════════════════════════════════════\n');
  
  // Show details if there are issues
  if (comparisonResults.summary.missingCount > 0) {
    console.log('❌ Missing Events (in ARD but not found in test):');
  ...
  if (comparisonResults.summary.parameterMismatchCount > 0) {
    console.log('⚠️  Parameter Mismatches (event found but parameters differ):');
  ...
  if (CONFIG.ARD_REPORT_GENERATION.html) {
    ...
    console.log(`   ✅ HTML Report: ${path.relative(__dirname, htmlPath)}`);
  }
  ...
  if (CONFIG.ARD_REPORT_GENERATION.csv) {
    ...
    console.log(`   ✅ CSV Report:  ${path.relative(__dirname, csvPath)}`);
  }
  ...
  if (generatedFiles.length === 0) {
    console.log('   ⚠️  No reports generated (all report types disabled in config/main.js)');
  }
  
  console.log('\n✅ ARD Comparison completed successfully!');
  
  // Exit with appropriate code based on results
  if (comparisonResults.summary.missingCount > 0 || comparisonResults.summary.parameterMismatchCount > 0) {
    console.log('\n⚠️  Issues found - review the reports for details');
    process.exit(0); // Still exit 0 for successful comparison (just with issues)
  }
  
} catch (error) {
  console.error('\n❌ Error during ARD comparison:', error.message);
